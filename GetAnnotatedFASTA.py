...
# Descript: A program that extracts the proteins annotations from a Genbank file and as well as some
...
def getProtienAnnotationFasta(seqRecord):
	...
	for feature in features:  # For each feature on the sequence
		if feature.type == "CDS":  # CDS means coding sequence (These are the only feature we're interested in)
			...
			# Gets the required qualifiers. Uses featQualifiers.get to return the qualifier or a default value if the qualifier
			...
			fasta.append(
				">" + protein_id + " " + gene + "-" + product + " (Locus: " + protein_locus + ")" + " (Location: " + location + ")" + "\n" + translated_protein + "\n")
