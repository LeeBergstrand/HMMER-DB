...
# SQL Query: Counts the number of distinct HMM families per organism source.
data = dbGetQuery(HMMDB, "SELECT
                              HMM_Data.HMM_Family,
	                            count(HMM_Data.HMM_Family) AS Count,
	                            substr(Organisms.Organism_Phylogeny,0,18) AS Phylum /* Feather the 33 to change the Phylogeny; Less:HigherTaxa <-> Greater:LowerTaxa */
                          FROM
	                            HMM_Data,
	                            HMM_Hits,
	                            Organisms,
